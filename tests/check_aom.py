...
GNN  = os.path.join(ROOT, "pipelines", "B_offline_models", "B4_graph_network", "expected", "aom.gnn")
...
# ---- the stoichiometric ratio, which was never a training target -------------------------------
#
# S says one sulfide per methane, exactly. In the SPECIES readout nothing enforces that -- each
# species reads its own rate off its own node -- and the shipped network of the previous release
# returned ratios between 0.76 and 1.11 over the upper 80% of the rate range, and between -1.57
# and 12.2 once the near-zero rates were included. In the EXTENT readout the species rates are
# formed as r = S xi, so the ratio is the stoichiometry at every input, and any deviation at all
# means the file and the evaluator disagree.
big = np.abs(pred[:, 0]) > 0.2 * np.abs(pred[:, 0]).max()
ratio = -pred[big, 2] / pred[big, 0]                       # HS produced over CH4 consumed
dev = float(np.max(np.abs(ratio - 1.0)))
print("  HS over CH4: %.6f .. %.6f over the upper 80%% of the rate range"
      % (float(ratio.min()), float(ratio.max())))

allr = -pred[:, 2] / np.where(np.abs(pred[:, 0]) > 1e-30, pred[:, 0], np.nan)
alldev = float(np.nanmax(np.abs(allr - 1.0)))
print("  and over EVERY sample, including the near-zero rates: worst deviation %.3g" % alldev)

exact = (M.get("readout", "species") == "extent")
if exact and alldev > 1e-9:
    print("\n** this file says readout extent, so the ratio should be exact and it is not **")
    sys.exit(1)
