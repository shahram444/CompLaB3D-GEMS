...
def main():
    ...
    ap.add_argument("--also", action="append", metavar="RXN_ID",
                    help="record this reaction's flux as an extra output column. "
                         "Repeatable. Use it for products the network should "
                         "predict, e.g. --also EX_ac_e for excreted acetate.")
    ap.add_argument("--growth-only", action="store_true",
                    help="record only the objective, as before. The resulting "
                         "network predicts growth and nothing else, and the "
                         "solver falls back to a Monod guess for consumption.")
    ...
    # ---- which fluxes to record --------------------------------------------
    # THE OBJECTIVE IS NOT THE ONLY THING THE LP COMPUTED.  A solve returns the
    # whole flux vector; recording only the objective throws away the realised
    # exchange fluxes, and the solver downstream then has to guess them with a
    # Monod term.  That guess is wrong whenever the swept bound is not the
    # binding constraint, which is most of the interesting domain.
    #
    # So by default every swept exchange is ALSO recorded as an output: the
    # flux the organism actually ran, not the bound it was allowed.  --also
    # adds any other reaction by id, for products the network should predict.
    extra_ids = list(args.also or [])
    extra_rxn = []
    for token in extra_ids:
        try:
            extra_rxn.append(model.reactions.get_by_id(token))
        except KeyError:
            sys.exit("ERROR: --also '%s' is not a reaction in this model." % token)

    if args.growth_only:
        out_names = ["growth"]
    else:
        out_names = (["growth"]
                     + ["r_%s" % r.id for r in rxn]
                     + ["r_%s" % r.id for r in extra_rxn])
    nout = len(out_names)
    ...
    # ---- the sweep ---------------------------------------------------------
    Y = np.zeros((len(X), nout))
    y = Y[:, 0]                       # column 0 stays the growth rate
    status = []
    t0 = time.time()
    for i, row in enumerate(X):
        ...
        ok = (st == "optimal" and sol.objective_value is not None)
        Y[i, 0] = float(sol.objective_value) if ok else 0.0
        if Y[i, 0] < 0.0:
            Y[i, 0] = 0.0
        if nout > 1:
            # Sign convention, and it matters: CompLaB works in POSITIVE =
            # consumed, cobra in negative-is-uptake.  The swept exchanges are
            # flipped here so the recorded column means the same thing as the
            # input column beside it.  Reactions added with --also are recorded
            # as cobra reports them, because a product should stay positive.
            col = 1
            for r in rxn:
                Y[i, col] = -float(sol.fluxes[r.id]) if ok else 0.0
                col += 1
            for r in extra_rxn:
                Y[i, col] = float(sol.fluxes[r.id]) if ok else 0.0
                col += 1
    ...
    if nout > 1:
        print("\nRecorded %d output column(s):" % nout)
        for k, nm in enumerate(out_names):
            print("  %-22s range %+.6g .. %+.6g" % (nm, Y[:, k].min(), Y[:, k].max()))
        print("A column that is flat at zero everywhere carries no information and")
        print("will only make the fit harder. Drop it with --growth-only or by")
        print("narrowing --also.")
    ...
    with open(args.output, "w") as f:
        ...
        for k, nm in enumerate(out_names):
            ...
            f.write("# output %d: %s, %s, range %.17g %.17g\n"
                    % (k, nm, unit, Y[:, k].min(), Y[:, k].max()))
        f.write("# samples: %d (%d optimal, %d growing)\n" % (len(X), nopt, ngrow))
        f.write(",".join(["v_in_%d" % k for k in range(nin)] + out_names) + "\n")
        for i in range(len(X)):
            f.write(",".join("%.17g" % v for v in X[i]) + ","
                    + ",".join("%.17g" % v for v in Y[i]) + "\n")

    print("Wrote %s  (%d rows, %d input column(s), %d output column(s))"
          % (args.output, len(X), nin, nout))
