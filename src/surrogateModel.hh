                          ...
                          plb::plint mask)
{
    ...
    static const double b5 = 0.85641685816650836571;
        ...
        /* THE TRAINING BOX, WRITTEN OUT.
         *
         * The two lines below are the same numbers the comment above describes,
         * as values the program can act on rather than prose a reader has to
         * act on. Outside this box the network extrapolates, and an
         * extrapolated growth rate looks exactly like a real one in the output
         * -- which is the one failure a fitted model has that a hand-written
         * rate law does not.
         *
         * clampCompiled() holds each input at the edge of the box and counts
         * how often it had to; the end of the run reports the percentage and
         * says plainly when it is large. Clamping rather than extrapolating is
         * the same choice the .sym and .gnn paths make, and for the same
         * reason: the value at the edge is at least a value the fit supports.
         *
         * IF YOU PASTE YOUR OWN WEIGHTS, UPDATE THESE TWO LINES. They are
         * lo = offset and hi = offset + (1 - x_ymin)/gain. Leaving someone
         * else's box here is worse than having none, because the report will
         * be reassuring and wrong. */
        static const double x_lo[2] = { x_offset0, x_offset1 };
        static const double x_hi[2] = { x_offset0 + (1.0 - x_ymin) / x_gain0,
                                        x_offset1 + (1.0 - x_ymin) / x_gain1 };
        ...
        /* Held to the training box first, and counted, so the report at the end
         * of the run can say how much of this simulation the network was
         * actually fitted for. */
        std::vector<double> xin(2);
        xin[0] = Fin[iM][0];                                   // substrate 0 (electron donor)
        xin[1] = Fin[iM][1];                                   // substrate 1 (electron acceptor)
        complab_srg::clampCompiled(xin, x_lo, x_hi, 2);

        std::vector<double> Xp0(2);
        Xp0[0] = (xin[0] - x_offset0) * x_gain0 + x_ymin;
        Xp0[1] = (xin[1] - x_offset1) * x_gain1 + x_ymin;
        ...
        /* -------------------------------------------------------------------
         * WHY Fout IS LEFT ALONE HERE, AND WHAT TO DO INSTEAD
         * -------------------------------------------------------------------
         * The shipped network has ONE output: the growth rate. So this routine
         * writes bioR and returns, leaving Fout holding the Fin the solver
         * pre-filled it with. The solver then works out consumption from a
         * Monod term instead.
         *
         * That fallback is exact only where the uptake bound WAS the binding
         * constraint. Take an organism given an acetate bound of 11.4 and an
         * Fe(III) bound of 57.1: if Fe(III) runs out first, the linear program
         * uses only 7.14 of the acetate. The Monod fallback draws all 11.4 --
         * 60% too much, in every voxel, every step. It also cannot release a
         ...
         *
         *     else if (microbeId == 0) {
         *         double o[srgnet_geobacter::nOutputs];
         *         srgnet_geobacter::evalAll(Fin[microbeId], o);
         *         bioR[microbeId]    = o[0];      // growth, 1/h
         *         Fout[microbeId][0] = o[1];      // substrate 0, mmol/gDW/h
         *         Fout[microbeId][1] = o[2];      // substrate 1
         *     }
         *
         * CHECK THE INDEX MAPPING ONCE, BY HAND. Fout[microbeId][j] is
         * substrate j in <name_of_substrates> order; o[k] is output k in the
         * order the exchanges were swept. Those agree only if you swept in
         * substrate order. A transposed pair is silent: both numbers stay
         * plausible and every result is wrong. The generated header prints the
         * mapping it expects in its own comment block; read it.
         *
         * The trainer reports each output separately, and you should read it
         * that way too. Growth is a smooth surface and fits easily. The flux
         * outputs are piecewise linear with kinks where the binding constraint
         * changes, and a network smooths exactly those, so they are the columns
         * that need more nodes and more samples.
         *
         * NO REBUILD ALTERNATIVE: the same choice is available at run time
         * through <surrogate><weights_file>, which reads a .srg holding either
         * kind. Switching between a growth-only and a growth-and-fluxes network
         * is then one line of XML. See config/CompLaB.reference.xml.
         * ------------------------------------------------------------------- */
