#!/usr/bin/env python3
"""
12_mixed_reaction_types  --  POST-PROCESSING

One organism running both a flux balance path and a compiled kinetics path in the
same voxels, in the same run. [v1.3] Not two competing populations, as this line
used to say: CompLaB.xml declares <number_of_microbes>1</number_of_microbes>, so
summary.csv carries one biomass column rather than two.
...
def main():
    ...
    if logs:
        ...
        for path in logs:
            try:
                ...
                pass
        for label, pattern in [('both rate paths', '(?i)\\[FBA\\]|kinetics'), ('diagnostics', '\\[DIAG\\]')]:
    ...
    print("-" * 78)
    print('The point of this case is the SHARED substrate budget. Both paths draw from the same voxel')
    print('under the same clamp, so neither can push a concentration negative by happening to run second.')
    print('')
    print('Check the substrate minima in the table above. Anything below zero means the clamp is not')
    print('being applied, which is a solver bug worth reporting rather than a tuning problem.')
