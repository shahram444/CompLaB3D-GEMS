#!/usr/bin/env python3
"""
02_diffusion_only  --  POST-PROCESSING

Pure diffusion. Peclet is 0, so the flow solver never ran.
...
def main():
    ...
    if logs:
        ...
        for path in logs:
            try:
                ...
                pass
        for label, pattern in [('the flow solver, which should have been skipped', '(?i)\\[NS\\]|peclet')]:
    ...
    print("-" * 78)
    print('The tracer is held at both ends, so a steady state is reached when its total stops')
    print('changing between rows. If the total is still moving at the last row, the run is too short.')
    print('')
    print('Compare the wall-clock time with example 01. The difference is the entire cost of the flow')
    print('solver, which is worth knowing before you scale up.')
