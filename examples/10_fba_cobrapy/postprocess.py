#!/usr/bin/env python3
"""
10_fba_cobrapy  --  POST-PROCESSING

The same linear program through COBRApy in an embedded interpreter.
...
Only closed species are balanced below. For this case:
    ...
    P            closed  (bounce-back both ends), so it can be balanced
"""
import csv
import os
import re
...
def main():
    ...
    if logs:
        ...
        for path in logs:
            try:
                ...
                pass
        for label, pattern in [('the interpreter and the model', '(?i)python|cobra'), ('diagnostics', '\\[DIAG\\]')]:
    ...
    print("-" * 78)
    print('This case exists to be compared with 09. Difference the two biomass fields.')
    print('')
    print('They should agree on uptake and growth, and differ where the model excretes strongly: the')
    print('COBRApy path clamps positive exchange draws to zero and does not run the repair loop GLPK')
    print('uses. A large disagreement anywhere else is a bug, not a back-end difference.')
    print('')
    print('[v1.3] GLPK and COBRApy organisms CAN now be mixed in one domain; the old')
    print('restriction against it has been removed. Each back end solves only its own')
    print('organisms and both add into the same increment lattices.')
    print('')
    print('The one thing not shared across the two is the cross-back-end exhaustion repair:')
    print('the re-solve against a pooled substrate budget when organisms over-draw runs')
    print('within a back end only. A substrate contested by a GLPK organism and a COBRApy')
    print('organism is therefore clamped per back end rather than renegotiated between')
    print('them. Give competing organisms the same back end if that matters.')
