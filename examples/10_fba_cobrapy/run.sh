...
#
# 10_fba_cobrapy
# The same metabolic model solved by COBRApy through an embedded Python.
#
...
#SBATCH --job-name=complab_10_fba_cobrapy
# the lab partition
...
#SBATCH --mem=32gb
# wall clock ceiling; 500 steps on this grid finishes well inside it
...
# The COBRApy path runs Python INSIDE the solver, and an embedded interpreter
# does not run the module system's EBPYTHONPREFIXES hook. So loading the module
# above is not enough on its own: the import fails at start-up with a missing
# mpmath. Naming the path explicitly is what fixes it.
export PYTHONPATH=/apps/eb/SciPy-bundle/2022.05-foss-2022a/lib/python3.10/site-packages:$PYTHONPATH
