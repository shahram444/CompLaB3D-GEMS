#!/usr/bin/env bash
#
# 10_fba_cobrapy  --  THE WHOLE PIPELINE
#
# The same linear program through COBRApy, as a cross-check on 09.
#
...
#     offline.sh       check the model loads in the interpreter COBRApy will use
...
# ---------------------------------------------------------------------------
echo
echo "== offline: check the model loads in the interpreter COBRApy will use"
./offline.sh
...
echo
echo "== building"
cmake -B build -S . -DCMAKE_BUILD_TYPE=Release -DENABLE_COBRAPY=ON
cmake --build build -j
