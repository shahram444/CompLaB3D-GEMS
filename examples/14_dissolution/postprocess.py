...
Only closed species are balanced below. For this case:
    ...
    Ca           closed  (bounce-back both ends), so it can be balanced
    calcite      closed  (bounce-back both ends), so it can be balanced
"""
import csv
import os
import re
...
def main():
    ...
    if missing:
        ...
        net = sum(c * (conserved(rows[-1], n) - conserved(rows[0], n)) for n, c in terms)
        scale = max(abs(conserved(rows[-1], n) - conserved(rows[0], n))
