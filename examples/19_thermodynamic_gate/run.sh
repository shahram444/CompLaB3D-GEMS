#!/bin/bash
#
# 19_thermodynamic_gate
# A reaction that stops where the energy runs out, gated by local dG.
#
...
#   ANME_*.vti
#   dG_ANME_*.vti             the local free energy, kJ/mol, negative where the reaction pays
#   FT_ANME_*.vti             the gate factor, 0 to 1, that the rate was multiplied by
#
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# THE CONTROL RUN: THE SAME CASE WITH THE GATE OFF
# ---------------------------------------------------------------------------
#
# This case only means something next to a run without the gate. Nothing else
# changes, one tag does. Do it as a SECOND job, from a copy of the folder, so
# the two sets of output do not overwrite each other:
#
#   cp -r . ../19_nogate
#   cd ../19_nogate
#   sed -i 's|<enabled>true</enabled>|<enabled>false</enabled>|' CompLaB.xml
#
# Check you changed the right tag before submitting, because <enabled> appears
# in more than one block:
#
#   grep -A2 '<thermodynamics>' CompLaB.xml
#
#   sbatch run.sh
#
# Then compare the two logs. postprocess.py will do it for you if you point it
# at both output directories; read its header for the exact invocation.
