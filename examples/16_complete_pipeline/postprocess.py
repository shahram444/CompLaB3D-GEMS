#!/usr/bin/env python3
"""
16_complete_pipeline  --  POST-PROCESSING

Geometry, chemistry, biology and genome-scale metabolism together.
...
Only closed species are balanced below. For this case:
    ...
    glucose      OPEN    (Dirichlet / Dirichlet), supplied from outside
    o2           OPEN    (Dirichlet / Dirichlet), supplied from outside
    acetate      closed  (bounce-back both ends), so it can be balanced
"""
import csv
import os
import re
...
BOUNDARY = {'glucose': 'Dirichlet / Dirichlet', 'o2': 'Dirichlet / Dirichlet', 'acetate': 'closed'}
...
def main():
    ...
    print("CLOSED-SPECIES REPORT -- acetate, the only closed species")
    for line in 'glucose and o2 are held at both ends.'.split("\n"):
        print("   %s" % line)
    terms = [('acetate', 1.0)]
    ...
    if logs:
        ...
        for path in logs:
            try:
                ...
                pass
        for label, pattern in [('how the model was resolved', '(?i)model|bigg|sbml|fetch'), ('the rate paths in use', '(?i)\\[FBA\\]|surrogate|kinetics'), ('diagnostics', '\\[DIAG\\]')]:
    ...
    print("-" * 78)
    print('This case has enough moving parts that no single field tells you whether it worked. The')
    print('conservation checks in <diagnostics> are the only honest verdict: name a closed sum for every')
    print('element you care about, and read the PASS or FAIL above.')
    print('')
    print('A check reported SKIPPED is NOT a pass. It means the substrate it names does not exist,')
    print('usually a typo, and nothing was ever verified. Treat a SKIPPED line as a failure of the')
    print('check, not of the run.')
