#!/usr/bin/env bash
#
# 16_complete_pipeline  --  THE WHOLE PIPELINE
#
# Geometry, chemistry, biology and metabolism together.
#
...
#     offline.sh       pre-fetch the metabolic model so the run is reproducible offline
...
# ---------------------------------------------------------------------------
echo
echo "== offline: pre-fetch the metabolic model so the run is reproducible offline"
./offline.sh
