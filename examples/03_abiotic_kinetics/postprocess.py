#!/usr/bin/env python3
"""
03_abiotic_kinetics  --  POST-PROCESSING

A chemical rate law with no organism. The reaction fires in every fluid voxel.
...
Only closed species are balanced below. For this case:
    ...
    A            OPEN    (Dirichlet / Neumann), supplied from outside
    B            OPEN    (Neumann / Dirichlet), supplied from outside
    C            closed  (bounce-back both ends), so it can be balanced
"""
import csv
import os
import re
...
BOUNDARY = {'A': 'Dirichlet / Neumann', 'B': 'Neumann / Dirichlet', 'C': 'closed'}
...
def main():
    ...
    print("CLOSED-SPECIES REPORT -- C, the product")
    for line in 'A enters from the left and B from the right, so only C is closed.'.split("\n"):
        print("   %s" % line)
    terms = [('C', 1.0)]
    ...
    print("-" * 78)
    print('C should build up in a BAND where the two fronts meet, not at either end. If C is highest')
    print('against a boundary, the rate law is firing where it should not be.')
    print('')
    print('This is also the case to run with the organisms switched off when your chemistry misbehaves.')
    print('A rate law is far easier to read without biomass moving underneath it.')
