#!/usr/bin/env python3
"""
01_flow_only  --  POST-PROCESSING

Steady flow through the slot. Nothing reacts, so the only question is whether the flow converged.
...
Only closed species are balanced below. For this case:
    ...
    tracer       OPEN    (Dirichlet / Dirichlet), supplied from outside
"""
import csv
import os
import re
...
BOUNDARY = {'tracer': 'Dirichlet / Dirichlet'}
...
def main():
    ...
    if logs:
        ...
        for path in logs:
            try:
                ...
                pass
        for label, pattern in [('the flow solver', '(?i)\\[NS\\]|converg|iteration cap')]:
    ...
    print("-" * 78)
    print('The tracer is held at BOTH ends, so its total is set by the boundaries, not by the run.')
    print('There is nothing to balance here and that is not a defect of the case.')
    print('')
    print('What matters is the [NS] block above: did the flow reach its tolerance, or did it stop at')
    print('the iteration cap? A run that hit the cap has not converged, and everything built on that')
    print('flow field inherits the error.')
