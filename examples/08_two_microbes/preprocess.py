#!/usr/bin/env python3
"""
08_two_microbes  --  PRE-PROCESSING

Builds the pore space this case runs on: two patches facing each other across the slot, one per population.
...
def build():
    ...
    # --- the initial biomass patch for material 4 ------------------------
    for x in range(NX):
        for y in range(NY):
            if ((x % 4) == 3 and 17 <= y <= 19):
                for z in range(NZ):
                    if g[x][y][z] == PORE:
                        g[x][y][z] = BIOFILM_4
